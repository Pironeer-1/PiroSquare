...
module.exports = {
  ...
  createNewRecruit: async (newPostData, userId, imagePath) => {
    ...
    const query =
      "INSERT INTO Post (title, content, board_type_id, user_id, post_image, category, member, start_date, end_date, likes_count, comments_count) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0);";
    let NewPost;
    if (newPostData.category == "study") {
      NewPost = await db.query(query, [
        xssTitle,
        xssContent,
        4,
        userId,
        ...
        1,
        newPostData.member,
        newPostData.start_date,
        newPostData.end_date,
      ]);
  ...
  updatePost: async (postId, newPostData, PostImage) => {
    ...
    const xssContent = xss(newPostData.content);
    const query =
      "UPDATE Post SET title=?, content=?, post_image=?, category, member, start_date, end_date, WHERE post_id=?;";
